...
## Next Session Should
Verify the **snapshot-vs-baseline precedence** (open-threads top item): if the main
cost path lets an adopted LiteLLM snapshot override the baseline, it REGRESSES the
now-vendor-verified deepseek/mistral-small rows to LiteLLM's stale values — may need a
pinned/verified-row concept. Also consider **releasing** #147–#150 (catalog now
vendor-accurate + framework improvements) if a version is wanted. Optional/older:
gemini-2.5 cache-read verified pass; Phase 2+ of ADR 0001 (SessionStart brief,
UserPromptSubmit guardrail, scorecard digest, Codex/Cursor Stop parity); user
